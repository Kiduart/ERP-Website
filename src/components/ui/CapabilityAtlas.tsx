...
export function CapabilityAtlas({ areas }: { areas: AtlasArea[] }) {
  ...
  // Visible cards only get stagger indices so brick-lay stays tight when filtering
  let brickIndex = 0;
  ...
  return (
    <div>
      ...
      <p
        className="mt-5 text-center text-sm font-medium text-brand-navy/70"
        aria-live="polite"
      >
        {activeGroup === "all"
      ...
      <InView once={false} key={activeGroup} className="home-rise mt-8">
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {areas.map((area) => {
            const tokens = ACCENTS[area.accent];
            const depth = Math.max(
              6,
              Math.round((area.featureCount / deepest) * 100),
            );
            ...
            const depthLabel =
              depth >= 70
                ...
                : depth >= 40
                  ? "Solid coverage"
                  : "Focused coverage";
            const stagger = visible ? Math.min(brickIndex++, 11) : 0;
            ...
            return (
              <article
                ...
                className={`home-brick home-brick-dense group relative ${
                  visible ? "flex" : "hidden"
                } flex-col overflow-hidden rounded-[1.75rem] border border-brand-navy/10 bg-white p-6 shadow-sm hover:-translate-y-1 hover:shadow-xl`}
              >
                <span
                  aria-hidden="true"
                ...
                <p className="mt-4 flex-1 text-sm leading-7 text-brand-navy/75">
                  {area.summary}
                </p>
