...
export function TeamPanelsHero({
  ...
  const panelImageAlts = [
    "Teachers collaborating on lesson plans with KIDUART school ERP",
    "School staff tracking tasks and projects in KIDUART dashboards",
    "Administrators reviewing student performance insights in KIDUART",
    "School teams coordinating daily operations on KIDUART",
    "School leadership monitoring campus-wide outcomes in KIDUART",
  ];
        ...
        <SectionReveal delay={0.08} className="mx-auto mt-14 grid max-w-6xl gap-5 md:grid-cols-5">
          {panelImages.map((panelImage, idx) => (
            ...
            >
              <img
                src={panelImage}
                alt={panelImageAlts[idx] ?? heroImageAlt(title)}
                className="h-72 w-full object-cover"
                {...lazyImgProps(IMAGE_DIMENSIONS.teamPanel)}
              />
