...
export const MATRIX_CATEGORIES: MatrixCategory[] = data.categories.map((category) => ({
  ...category,
...
export function findMatrixModule(
  ...
  return category?.modules.find((entry) => entry.sheetName === name || entry.name === name);
}
export const MATRIX_TOTALS: MatrixTotals = data.totals;

export const MATRIX_CATEGORY_SLUGS = MATRIX_CATEGORIES.map((category) => category.slug);
...
export function countSubModules(category: MatrixCategory): number {
  return category.modules.reduce((sum, module) => sum + module.subModules.length, 0);
}
...
export function toPublicSubModule(subModule: MatrixSubModule): PublicSubModule {
  const total = subModule.features.length;
  // A single hidden row is not worth a "more" line, so show it instead.
  const shown =
    total <= PUBLIC_FEATURE_SAMPLE + 1
      ? subModule.features
      : subModule.features.slice(0, PUBLIC_FEATURE_SAMPLE);
...
export function toPublicModule(module: MatrixModule): PublicModule {
  const subModules = module.subModules.map(toPublicSubModule);
  ...
  return {
    ...
    subModules,
    hiddenFeatureCount: subModules.reduce((sum, entry) => sum + entry.hiddenFeatureCount, 0),
  };
}

/** Largest modules first — used to lead with the deepest capability areas. */
export function topModules(category: MatrixCategory, limit = 6): MatrixModule[] {
  return [...category.modules].sort((a, b) => b.featureCount - a.featureCount).slice(0, limit);
}
