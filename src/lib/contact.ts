...
function getCountryLabel(iso2: string) {
  return COUNTRY_NAME_OVERRIDES[iso2] ?? COUNTRY_NAMES[iso2] ?? iso2;
}

export const COUNTRY_CODES: CountryCodeOption[] = getCountries()
  .map((iso2) => {
    ...
    return {
      iso2,
      value: callingCode,
      label: `${getCountryLabel(iso2)} (${callingCode})`,
    };
  })
  .reduce<CountryCodeOption[]>((items, option) => {
  ...
  .sort((a, b) => {
    if (a.value === DEFAULT_COUNTRY_CODE) return -1;
    if (b.value === DEFAULT_COUNTRY_CODE) return 1;
