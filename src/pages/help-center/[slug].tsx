...
const helpArticlesData: Record<string, any> = {
  "getting-started": {
    title: "Getting Started with KIDUART",
  ...
  "fee-setup": {
    title: "Configuring Fee Management",
    category: "Account Setup",
  ...
  "parent-portal-guide": {
    title: "Guide to the Parent Portal",
    category: "Getting Started",
...
export default function HelpArticle() {
    ...
    return (
      <PageTransition className="pt-32 pb-24 text-center min-h-[60vh] flex flex-col items-center justify-center">
        ...
        <p className="text-xl text-brand-navy/70 mb-8">
          The help article you are looking for doesn't exist.
        </p>
        <Link
          href="/help"
          className="px-8 py-4 rounded-full bg-brand-navy text-white font-bold inline-flex items-center gap-2"
        >
  ...
  if (relatedArticles.length < 3) {
    const more = Object.entries(helpArticlesData)
      .filter(
        ([key]) => key !== slug && !relatedArticles.find(([k]) => k === key),
      )
    ...
    return text.split("\\n").map((line, index) => {
      ...
      if (line.startsWith("## ")) {
        return (
          <h2
            key={index}
            className="text-2xl font-bold text-brand-navy mt-10 mb-4"
          >
            {line.replace("## ", "")}
          </h2>
        );
      ...
      if (line.startsWith("- ")) {
        return (
          <li
            key={index}
            className="ml-6 mb-2 text-brand-navy/80 text-lg list-disc"
          >
            {line
              .replace("- ", "")
      ...
      if (/^\\d+\\.\\s/.test(line)) {
        return (
          <li
            key={index}
            className="ml-6 mb-2 text-brand-navy/80 text-lg list-decimal"
          >
            {line
              .replace(/^\\d+\\.\\s/, "")
              .replace(/\\*\\*(.*?)\\*\\*/g, "<strong>$1</strong>")}
      ...
      if (parts.length > 1) {
        return (
          <p key={index} className="text-lg text-brand-navy/80 mb-4">
            {parts.map((part, i) =>
              i % 2 === 1 ? (
                <strong key={i} className="text-brand-navy font-bold">
                  {part}
                </strong>
              ) : (
                part
  ...
  return (
    <>
      <PageSeoHead
        title={`${article.title} | School ERP Help | KIDUART`}
        description={`${article.title}  KIDUART school ERP and school management software help guide for Indian schools. Category: ${article.category}.`}
        path={`/help-center/${slug}`}
        keywords={`${article.title.toLowerCase()}, school ERP help, school management software guide, KIDUART ${article.category.toLowerCase()}`}
      />
      <PageTransition className="pt-20 pb-0">
        ...
        <section className="section-space-tight bg-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <SectionReveal
              delay={0.2}
              className="mt-16 pt-8 border-t border-brand-navy/10 flex flex-col sm:flex-row items-center justify-between gap-6"
            >
              ...
              {feedback && (
                <p className="text-brand-teal font-medium text-sm mt-2 sm:mt-0 w-full sm:w-auto text-center">
            ...
            <div className="space-y-4">
              {relatedArticles.map(([key, data], idx) => (
                <SectionReveal key={key} delay={idx * 0.1}>
                  <Link
                    href={`/help/${key}`}
                    className="block bg-white p-6 rounded-2xl border border-brand-navy/5 shadow-sm hover:shadow-md hover:border-brand-teal/30 transition-all group flex items-center justify-between"
                  >
                    <div>
          ...
          <div className="max-w-2xl mx-auto px-4">
            <SectionReveal>
              ...
              <a
                ...
                className="px-8 py-4 rounded-full bg-brand-teal text-white font-bold text-lg hover:bg-white hover:text-brand-teal transition-all shadow-lg inline-flex items-center gap-2"
              >
