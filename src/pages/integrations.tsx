...
export default function Integrations({
  ...
  return (
    <PageTransition className="pt-20 pb-0">
      ...
      />
      <SchemaMarkup
        data={buildFaqPageSchema({ integrations: INTEGRATION_FAQS })}
      />
        ...
        <div className="page-shell">
          <SectionReveal className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-brand-navy/[0.76]">
              Most vendor integration pages show a grid of brand marks and let
              you assume everything works. If an integration decides whether
              KIDUART fits your school, you should know where it stands before
              the demo not after signing.
            </p>
          </SectionReveal>

          <Stagger
            className="mt-10 grid gap-5 md:grid-cols-3"
            itemClassName="motion-stamp rounded-[1.5rem] border border-brand-navy/[0.1] bg-brand-beige/25 p-6"
          >
            {ledger.map((entry) => {
              ...
              return (
                <div key={entry.status}>
                  <div className="flex items-center gap-2.5">
                    ...
                    />
                    <span
                      className={`text-xs font-bold uppercase tracking-[0.16em] ${style.accentText}`}
                    >
                      {meta.long}
                    </span>
                  </div>
                  <p className="mt-4 text-4xl font-extrabold leading-none text-brand-navy">
                    {entry.count}
                  </p>
      ...
      <section className="section-space relative overflow-hidden bg-brand-beige/20">
        ...
        <div className="page-shell relative z-10">
          <InView once className="motion-rise mb-12 max-w-3xl">
            <p className="section-kicker">The full directory</p>
            <h2 className="mt-4 text-3xl font-bold text-brand-navy md:text-4xl">
              Connectors grouped by the job they do
            </h2>
            <p className="mt-4 text-base leading-8 text-brand-navy/[0.76]">
              Filter by status or jump to the category you care about. Every
              connector page explains the hop-by-hop flow, who holds the
              credentials, what your school has to bring, and which modules the
              data actually lands in.
            </p>
          </InView>
      ...
      <section className="section-space border-y border-brand-navy/5 bg-white">
        <div className="page-shell">
          <SectionReveal className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-brand-navy/[0.76]">
              The same five rules apply whether it is a fee payment, an absence
              SMS or a gate punch. This is the part a school should ask every
              vendor about, because it decides who is liable when something goes
              wrong.
            </p>
          </SectionReveal>
          ...
          <div className="relative mt-12">
            <span
              aria-hidden="true"
              className="wire-rail pointer-events-none absolute left-6 right-6 top-5 hidden lg:block"
            />
            <Stagger
              as="ol"
              className="relative grid gap-8 md:grid-cols-2 lg:grid-cols-5 lg:gap-5"
              itemClassName="motion-spine-node relative lg:px-1"
            >
              {CONNECTION_STEPS.map((step, index) => (
                <li key={step.title}>
                  <article className="relative pl-4 lg:pl-5">
                    <span
                      aria-hidden="true"
                    ...
                    <p className="mt-2 text-sm leading-7 text-brand-navy/[0.76]">
                      {step.detail}
                    </p>
                  </article>
                </li>
      ...
      <section className="section-space-tight bg-brand-beige/30">
        <div className="page-shell">
          <SectionReveal className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-brand-navy/[0.76]">
              These come up in almost every demo. Rather than leave you to
              discover them later, here is the list with what we do instead.
            </p>
          </SectionReveal>
              ...
              <li key={entry.name} className="h-full">
                ...
                >
                  ...
                  <span>
                    ...
                    <span className="mt-1.5 block text-sm leading-7 text-brand-navy/[0.78]">
                      {entry.note}
                    </span>
                  </span>
                </SectionReveal>
        ...
        <div className="page-shell relative z-10">
          <div className="grid items-center gap-12 lg:grid-cols-2">
            <SectionReveal>
              ...
              <h2 className="mb-6 mt-4 text-3xl font-bold lg:text-4xl">
                Build your own with the REST API
              </h2>
              <p
                className="mb-8 text-lg leading-8"
                style={{ color: "rgba(252,246,211,0.82)" }}
              >
                Schools connect attendance devices, their public website, or a
                reporting warehouse. Your developers work against versioned REST
                endpoints using a scoped API key never a shared staff login with
                rate limits and request logging on every call.
              </p>
              <ul className="mb-8 space-y-3.5">
                {[
                  ...
                  <li key={item} className="flex items-start gap-3">
                    <Zap
                      className="mt-1 h-5 w-5 shrink-0 text-brand-yellow"
                      aria-hidden="true"
                    />
                    <span>{item}</span>
                  </li>
...
> = async () => {
  ...
  const categories: DirectoryCategory[] = INTEGRATION_CATEGORIES.map(
    (category) => ({
      slug: category.slug,
