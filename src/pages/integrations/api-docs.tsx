...
import { AlertTriangle, ArrowRight, ChevronDown, KeyRound, Radio, ShieldCheck, Terminal } from "lucide-react";
...
const API_GROUPS: ApiGroup[] = [
  {
    ...
    endpoints: [
      { method: "GET", path: "/api/v1/students", purpose: "List students with class, section and status filters.", scope: "students:read" },
      { method: "GET", path: "/api/v1/students/{id}", purpose: "Fetch one student with the fields your key is allowed to see.", scope: "students:read" },
      { method: "POST", path: "/api/v1/students", purpose: "Create a student record from an external admission form.", scope: "students:write" },
      { method: "PATCH", path: "/api/v1/students/{id}", purpose: "Update contact or guardian details without touching academic history.", scope: "students:write" },
    ],
  },
  {
    id: "attendance",
    ...
    endpoints: [
      { method: "POST", path: "/api/v1/attendance/biometric", purpose: "Submit device punches using a device key rather than a staff login.", scope: "attendance:write" },
      { method: "GET", path: "/api/v1/attendance", purpose: "Read the register for a class, section or date range.", scope: "attendance:read" },
      { method: "POST", path: "/api/v1/attendance", purpose: "Mark or correct attendance from an approved external system.", scope: "attendance:write" },
    ],
  },
  {
    id: "fees",
    ...
    endpoints: [
      { method: "GET", path: "/api/v1/fees/dues", purpose: "Outstanding dues by student, class or fee head.", scope: "fees:read" },
      { method: "GET", path: "/api/v1/fees/transactions", purpose: "Collection history for reconciliation and reporting.", scope: "fees:read" },
      { method: "POST", path: "/api/v1/fees/collect", purpose: "Record a payment collected in an approved external channel.", scope: "fees:write" },
    ],
  },
  {
    id: "academics",
    ...
    endpoints: [
      { method: "GET", path: "/api/v1/classes", purpose: "Classes and sections for the current academic session.", scope: "academics:read" },
      { method: "GET", path: "/api/v1/subjects", purpose: "Subject master with class mapping.", scope: "academics:read" },
      { method: "GET", path: "/api/v1/exams/results", purpose: "Published assessment results for reporting use.", scope: "academics:read" },
    ],
  },
  {
    id: "staff",
    ...
    endpoints: [
      { method: "GET", path: "/api/v1/staff", purpose: "Staff directory with department and designation.", scope: "staff:read" },
      { method: "GET", path: "/api/v1/staff/attendance", purpose: "Staff attendance for a period, for payroll input.", scope: "staff:read" },
    ],
  },
  {
    id: "reports",
    ...
    endpoints: [
      { method: "GET", path: "/api/v1/reports/{type}", purpose: "Fetch a generated report with the filters you pass.", scope: "reports:read" },
      { method: "POST", path: "/api/v1/reports/export", purpose: "Queue a large export and collect it when it is ready.", scope: "reports:read" },
    ],
  },
];

const WEBHOOKS_IN = [
  { provider: "Razorpay", events: "payment.captured, payment.failed, refund.processed, order.paid, dispute.created" },
  { provider: "Stripe", events: "payment_intent.succeeded, charge.failed, charge.refunded" },
  { provider: "Twilio and MSG91", events: "SMS delivery and failure callbacks" },
  { provider: "SendGrid and Mailgun", events: "delivered, bounced, complaint" },
  { provider: "WhatsApp Business", events: "message status callbacks and inbound message events" },
];

const ERRORS = [
  { code: "400", meaning: "The request body or query failed validation. The response names the field." },
  { code: "401", meaning: "Missing, expired or revoked credentials." },
  { code: "403", meaning: "Authenticated, but the key's scope does not cover this action." },
  { code: "404", meaning: "The record does not exist, or does not belong to your school." },
  { code: "409", meaning: "The action conflicts with the current state, such as a duplicate receipt." },
  { code: "429", meaning: "Rate limit hit. Back off and retry using the returned headers." },
  { code: "500", meaning: "Something failed on our side. It is logged with a reference you can quote to support." },
];
  ...
  return (
    <>
      ...
      <PageTransition className="pt-20 pb-0">
        <section className="relative overflow-hidden bg-brand-navy" style={{ color: "#fcf6d3" }}>
          <BackgroundBlobs
            blobs={[
              { color: "#0c716b", size: 400, position: "top-left", opacity: 0.18 },
              { color: "#fcbf49", size: 360, position: "bottom-right", opacity: 0.14 },
            ]}
          />
            ...
            <div className="grid gap-10 lg:grid-cols-[1.15fr_1fr] lg:items-center">
              <SectionReveal>
                <span className="inline-flex items-center gap-2 rounded-full border border-brand-yellow/40 bg-brand-yellow/[0.14] px-3 py-1 text-[0.7rem] font-bold uppercase tracking-[0.16em] text-brand-yellow">
                  <Terminal className="h-3.5 w-3.5" aria-hidden="true" /> Developer reference
                </span>
                ...
                <p className="mt-5 max-w-2xl text-lg leading-8" style={MUTED}>
                  Versioned endpoints under a stable base path, scoped API keys instead of a shared staff
                  login, signature-verified provider webhooks and a predictable error contract. This page is
                  the overview your IT team needs before asking for access; the full endpoint reference is
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  ...
                  >
                    Request API access <ArrowRight className="h-4 w-4" aria-hidden="true" />
                  </Link>
                  <Link
                    href="/integrations"
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {[
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-7 text-brand-navy/[0.78]">{item.body}</p>
                </SectionReveal>
              ))}
            </div>
          </div>
          ...
          <div className="page-shell">
            <SectionReveal className="max-w-3xl">
              ...
              <p className="mt-4 text-base leading-8 text-brand-navy/[0.78]">
                Machine-to-machine integrations use an API key issued to your school. Anything acting on
                behalf of a signed-in person uses that person&apos;s session token, so their role decides what
                they can see. Do not build a server integration on a staff login — it breaks the moment that
                person leaves.
              </p>
            </SectionReveal>

            <div className="mt-10 grid gap-6 lg:grid-cols-2">
              <SectionReveal className="rounded-[1.5rem] border border-brand-navy/[0.1] bg-white p-6">
                <h3 className="text-lg font-bold text-brand-navy">API key — server to server</h3>
                <p className="mt-2 text-sm leading-7 text-brand-navy/[0.78]">
                  Issued per integration with named scopes. Send it as a header on every request, keep it on
                  your server, and rotate it when the person who owns the integration changes.
                </p>
...
Content-Type: application/json`}
              ...
              <SectionReveal delay={0.1} className="rounded-[1.5rem] border border-brand-navy/[0.1] bg-white p-6">
                <h3 className="text-lg font-bold text-brand-navy">Bearer token — on behalf of a user</h3>
                <p className="mt-2 text-sm leading-7 text-brand-navy/[0.78]">
                  Used by the web and mobile apps after sign-in. Access tokens are short-lived and refreshed,
                  and every call is still filtered by that user&apos;s role and school.
                </p>
                <pre className="mt-4 overflow-x-auto rounded-xl border border-brand-navy/10 bg-[#0d1117] p-4 font-mono text-xs leading-6 text-gray-300">
...
Content-Type: application/json`}
            ...
            <SectionReveal className="mt-6 flex items-start gap-3 rounded-2xl border border-brand-orange/25 bg-brand-orange/[0.07] p-5">
              ...
              <p className="text-sm leading-7 text-brand-navy/[0.82]">
                All traffic is HTTPS only. Keys are shown once when issued and stored hashed on our side, so
                if it is lost we reissue rather than retrieve — which is exactly what you want from anything
                that can read student data.
              </p>
            </SectionReveal>
          </div>
        </section>
        ...
        <section className="section-space relative overflow-hidden bg-brand-navy" style={{ color: "#fcf6d3" }}>
          ...
          <div className="page-shell relative z-10">
            <SectionReveal className="max-w-3xl">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-yellow">Endpoint groups</p>
              ...
              <p className="mt-4 text-base leading-8" style={MUTED}>
                Representative endpoints for the integrations schools ask for most. Paths, payloads and the
                full field list arrive with your key as a reference document, so your developer builds
                against the current contract rather than a marketing page that drifted.
              </p>
            </SectionReveal>
            ...
            <div className="mt-10 grid gap-6 lg:grid-cols-2">
              <SectionReveal className="rounded-[1.5rem] border border-white/12 bg-white/[0.05] p-6">
                <h3 className="text-lg font-bold text-brand-beige">Pagination and filtering</h3>
                <p className="mt-2 text-sm leading-7" style={MUTED}>
                  List endpoints are paged. Pass <code className="font-mono text-brand-yellow">page</code> and{" "}
                  <code className="font-mono text-brand-yellow">limit</code>, read the totals from{" "}
                  <code className="font-mono text-brand-yellow">meta</code>, and filter with the query
                  parameters documented for that resource. Large exports are queued rather than streamed, so a
                  full-year report never times out mid-request.
                </p>
              </SectionReveal>

              <SectionReveal delay={0.1} className="rounded-[1.5rem] border border-white/12 bg-white/[0.05] p-6">
                <h3 className="text-lg font-bold text-brand-beige">Rate limits</h3>
                <p className="mt-2 text-sm leading-7" style={MUTED}>
                  Requests are rate limited per key under a fair-use policy, with stricter limits on
                  authentication routes. A limited response returns 429 with headers telling you when to
                  retry. If a genuine workload needs more headroom, tell us the pattern and we will size it
                  with you rather than making you discover the ceiling in production.
                </p>
              </SectionReveal>
            </div>
          </div>
          ...
          <div className="page-shell">
            <div className="grid gap-12 lg:grid-cols-2">
              <SectionReveal>
                ...
                <p className="mt-4 text-base leading-8 text-brand-navy/[0.78]">
                  Incoming webhooks are how a payment gets confirmed and a message gets marked delivered.
                  Each one is signature-verified against the provider&apos;s secret before anything is written,
                  and a repeated callback updates the existing record instead of duplicating it.
                </p>
                <ul className="mt-6 space-y-3">
                  {WEBHOOKS_IN.map((hook) => (
                    ...
                    >
                      <p className="text-sm font-bold text-brand-navy">{hook.provider}</p>
                      <p className="mt-1 font-mono text-xs leading-6 text-brand-navy/[0.76]">{hook.events}</p>
                    </li>
                  ))}
                </ul>
                <p className="mt-6 rounded-2xl border border-brand-orange/25 bg-brand-orange/[0.07] p-5 text-sm leading-7 text-brand-navy/[0.82]">
                  <strong className="font-bold text-brand-navy">Not available yet:</strong> subscribing your own
                  endpoint to KIDUART events. Outbound webhooks are on the roadmap. Until they ship, poll the
              ...
              <SectionReveal delay={0.12}>
                ...
                <h2 className="mt-4 text-3xl font-bold text-brand-navy">One error shape, every endpoint</h2>
                <p className="mt-4 text-base leading-8 text-brand-navy/[0.78]">
                  Failures come back in the same envelope as successes, so your client handles one shape
                  rather than guessing per route.
                </p>
                ...
                <dl className="mt-6 divide-y divide-brand-navy/[0.08] rounded-2xl border border-brand-navy/[0.1]">
                  {ERRORS.map((error) => (
                    <div key={error.code} className="flex gap-4 p-4">
                      <dt className="w-12 shrink-0 font-mono text-sm font-bold text-brand-teal">{error.code}</dt>
                      <dd className="text-sm leading-6 text-brand-navy/[0.8]">{error.meaning}</dd>
                    </div>
                  ))}
                </dl>
              </SectionReveal>
        ...
        <section className="section-space bg-brand-beige/25">
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <SectionReveal className="text-center">
              ...
              <h2 className="mt-4 text-3xl font-bold text-brand-navy">Developer questions, answered</h2>
            </SectionReveal>
